...
$(document).ready(function () {
  ...
  var _propertyBookings = undefined;
  var _bookings = [];
  var _properties = [];
  ...
  function getPath() {
    var name = slug.slice(-1).toString();
  ...
  function getPropertiesEnd(dateText) {
    var endingDate = $('#end-filter').datepicker('getDate');
    $.ajax({
      url: _path,
      type: 'GET',
      data: { startDate: parseDate(dateText),
        endDate: parseDate($.datepicker.formatDate('mm/dd/yy', endingDate)) },
  ...
  function getPropertiesStart(dateText) {
    var startDate = $('#start-filter').datepicker('getDate');
    $.ajax({
      url: _path,
      type: 'GET',
      data: { startDate: parseDate($.datepicker.formatDate('mm/dd/yy', startDate)),
  ...
  function hideProperties() {
    $properties.each(function (index, property) {
      var $divTitle = $(property).find('a').text().split('#')[0];
...
$(document).ready(function () {
  var $props = $('.item');
  var $prop = undefined;
  var _propName = undefined;
  var $propData = undefined;

  $('#property_filter_name').on('keyup', function () {
    _propName = this.value.toLowerCase();
    $props.each(function (index, property) {
      $prop = $(property);
...
$(document).ready(function () {
  var $items = $('.item');
  var slug = window.location.pathname.split('/').slice(-1).toString();
  ...
  $(function () {
    $('#slider-range').slider({
      ...
      slide: function slide(event, ui) {
        ...
        $.getJSON('/api/v1/' + slug + '.json', function (response) {
          start(response);
        });
